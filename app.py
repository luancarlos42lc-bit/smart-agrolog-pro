...
# ---------------------------------------------------------
    ...
    if uploaded_drone is not None:
        try:
            ...
            st.image(image_drone, caption="Mosaico Aéreo Carregado e Otimizado", use_container_width=True)
            ...
            if st.button("🚀 Processar Varredura e Gerar Mapa de Calor"):
                with st.spinner("Analisando densidade foliar e gerando gradientes de severidade..."):
                    ...
                    with col_res1:
                        st.image(resultado_final, caption="Mapa Térmico (Transparência Fluidificada)",
            ...
            st.error(f"Erro ao processar imagem do talhão: {e}")